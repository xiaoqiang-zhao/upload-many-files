...
module.exports = {
    ...
    async pipe(program, isDirect) {
        if (program.start || isDirect) {
            ...
            if (program.uploadParams) {
                const uploadParams = program.uploadParams;
                try {
                    const uploadParamsArr = uploadParams.split(',');
    ...
    async writeJobsFile(jobsMain, groupIndex) {
        ...
        try {
            await fsPromises.writeFile(`${dataFolderPath}jobs-${(groupIndex + '').padStart(3, '0')}.json`, content, {
                encoding: 'utf-8'
            }).then(() => {
                jobsMain.currentJobsList = [];
            }, error => {
    ...
    async startUploadFiles(config, jobsMain) {
        ...
        async function updateJobsUpload() {
            ...
            if (currentUploadingJobsCount === 0) {
                if (recycleJobs.length > 0) {
                    console.log(chalk.red('\n >>>>> 这里应该有问题 <<<<<< \n'));
                    console.log(chalk.red('\n >>>>> 这里应该有问题 <<<<<< \n'));
                    recycleJobs.forEach(item => {
                ...
                const indexStr = (jobsMain.uploadedGroupIndex + 1 + '').padStart(3, '0');
                jobsMain.currentJobsPath = `${dataFolderPath}jobs-${indexStr}.json`;
         ...
         */
        async function postFile(index, retryNumberOfTimes) {
            ...
            request.post({
                ...
                if (!error && isSuccess) {
                    if (jobsMain.currentJobsList[index].isUploaded === false) {
                        ...
                        uploadedJobsTotal++;
                        log.trace(`uploadedJobsTotal 计数累加成功，path: ${path}`);
                    ...
                    console.log(chalk.red(`\n ${text}，已发起第【${retryNumberOfTimes + 1}】次重试`));
                    log.trace(text, 'error:', error, 'body:', body);
                    // 重试上传
                    postFile(index, retryNumberOfTimes + 1);
                }
            });
        }
