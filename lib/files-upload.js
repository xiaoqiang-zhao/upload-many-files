...
module.exports = {
    ...
    async updateJobsUpload() {
        ...
        if (this.data.totalUploadGroup === jobsMain.uploadedGroupIndex) {
            console.log(chalk.green(` \n >>>>> 已全部上传完成, 共计: ${jobsMain.jobsTotal} <<<<<< \n`));
            return;
        }
        // 放入新一批数据
    ...
    async postFile(index, retryNumberOfTimes) {
        ...
        request.post({
            ...
            if (!error && isSuccess) {
                if (this.data.jobsMain.currentJobsList[index].isUploaded === false) {
                    this.data.jobsMain.currentJobsList[index].isUploaded = true;
                    this.data.uploadedJobsTotal++;
                ...
                if (this.data.uploadedJobsTotal % uploadConfig.capacity === 0) {
                    this.data.jobsMain.uploadedGroupIndex++;
                    console.log('\n uploadedJobsTotal: ', this.data.uploadedJobsTotal);
                }
                
                spinner.setSpinnerTitle(`正在上传: ${this.data.uploadedJobsTotal}/${this.data.jobsMain.jobsTotal}`);
                if (this.data.uploadedJobsTotal < this.data.jobsMain.jobsTotal) {
                    this.updateJobsUpload();
                }
                else {
                    spinner.stop(true);
                    this.saveUploadingProcess();
